...
def test_distribute(a: float, b: float, c: float) -> None:
    ...
    """
    assert is_close(mul(c, add(a, b)), add(mul(a, c), mul(b, c)))
...
def test_sum_distribute(ls1: List[float], ls2: List[float]) -> None:
    ls3 = [add(val1, val2) for val1, val2 in zip(ls1, ls2)]
    for val, val3 in zip(addLists(ls1, ls2), ls3):
        assert_close(val, val3)
